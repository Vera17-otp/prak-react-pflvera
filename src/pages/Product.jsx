...
// ✅ Jalur diperbaiki: mengambil dari ../data/product.json sesuai struktur foldermu
...
export default function Products({ searchTerm = "" }) {
    ...
    // Mengamankan searchTerm agar jika bernilai undefined tidak menyebabkan crash
    const safeSearchTerm = typeof searchTerm === "string" ? searchTerm : "";

    // Memfilter produk berdasarkan properti 'tittle' dari file JSON kamu
    const filtered = Array.isArray(productsData) 
        ? productsData.filter((p) =>
            p.tittle && p.tittle.toLowerCase().includes(safeSearchTerm.toLowerCase())
          )
        : [];
    ...
    return (
        <div className="min-h-screen bg-[#FDFDFD] p-8 md:p-12 relative overflow-hidden text-slate-900 font-sans">
            ...
            <div className="relative z-10 max-w-7xl mx-auto">
                ...
                <PageHeader
                    title="Products"
                    breadcrumb={["Operations", "Products"]}
                >
                        ...
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="border-b border-slate-50">
                                    <th className="px-10 py-6 text-[10px] uppercase tracking-[0.2em] font-black text-slate-400">Code</th>
                                    <th className="px-10 py-6 text-[10px] uppercase tracking-[0.2em] font-black text-slate-400">Product Name</th>
                                    <th className="px-10 py-6 text-[10px] uppercase tracking-[0.2em] font-black text-slate-400">Category</th>
                                    <th className="px-10 py-6 text-[10px] uppercase tracking-[0.2em] font-black text-slate-400 text-center">Stock</th>
                                    <th className="px-10 py-6 text-[10px] uppercase tracking-[0.2em] font-black text-slate-400 text-right">Price</th>
                                            ...
                                            <td className="px-10 py-6">
                                                <div className="flex items-center gap-4">
                                                    <div className="w-10 h-10 rounded-xl bg-slate-100 flex items-center justify-center font-black text-slate-400 text-sm border border-white shadow-sm group-hover:bg-indigo-600 group-hover:text-white transition-all">
                                                        {p.tittle ? p.tittle.charAt(0) : "?"}
                                                    </div>
                                                    <div>
                                                        <Link  to={`/products/${p.id}`} className="font-bold text-slate-800 text-sm tracking-tight">{p.tittle}</Link>
                                                        <div className="text-[10px] text-slate-400 font-bold uppercase tracking-tighter">{p.brand}</div>
                                                    </div>
                                                </div>
                                            </td>
                                            {/* Category Column */}
                                            <td className="px-10 py-6">
                                                <span className="text-xs font-semibold text-slate-600 bg-slate-100/60 px-3 py-1 rounded-xl">
                                                    {p.category}
                                                </span>
                                            </td>
                                            {/* Stock Column with dynamic badges */}
                                            <td className="px-10 py-6 text-center">
                                                <span className={`inline-flex items-center px-4 py-1.5 rounded-2xl text-[10px] font-black uppercase tracking-widest border
                                                    ${p.stock > 20 
                                                        ...
                                                        : p.stock <= 20 && p.stock > 8
                                                        ? 'bg-amber-50 text-amber-600 border-amber-100' 
                                                        : 'bg-rose-50 text-rose-600 border-rose-100'}`}>
                                                    {p.stock} Pcs
                                                </span>
                                            </td>
                                            {/* Price Column formatted to Rupiah */}
                                    ...
                                    <tr>
                                        <td colSpan="6" className="px-10 py-32 text-center">
                                            <div className="flex flex-col items-center justify-center grayscale opacity-20">
                                                ...
                                                <p className="text-2xl font-black uppercase tracking-widest italic">No Products Found</p>
                                            </div>
                                        </td>
                                    </tr>
                                )}
                    ...
                    <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
                        ...
                        <div className="relative bg-white/90 border border-white rounded-[3rem] shadow-[0_50px_100px_-20px_rgba(0,0,0,0.3)] max-w-md w-full p-12 transform transition-all animate-in zoom-in duration-300">
                            <div className="mb-10 text-center">
                                ...
                                <h2 className="text-3xl font-black text-slate-900 tracking-tighter italic">Add Product</h2>
                                <p className="text-slate-400 text-sm mt-2">Create a new item in your inventory.</p>
                            </div>

                            <div className="space-y-6">
                                <div className="group">
                                    <label className="block text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 mb-2 ml-2 transition-colors group-focus-within:text-indigo-600">Product Title</label>
                                    <input 
                                        type="text"
                                ...
                                <div className="group">
                                    <label className="block text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 mb-2 ml-2 transition-colors group-focus-within:text-indigo-600">Price</label>
